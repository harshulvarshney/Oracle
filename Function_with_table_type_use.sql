...
BEGIN
	...
	FOR I IN (SELECT MIN(EFF_DATE) AS EFF_DATE,MAX(END_DATE) AS END_DATE,CLIENTIDENTIFIER,ORG_ID,COMPANY_NAME,EIN,ENROLLMENT_SETUP_IDENTIFIER,
            ...
            FROM CLIENT C
            JOIN EMPLOYER_ORGANIZATION EO ON EO.EIN = C.EXT_CLIENT_ID
            JOIN ENROLLMENT_SETUP ES ON ES.EMPLOYER_ORGANIZATION_ID = EO.ID
            JOIN ENROLLMENT_PERIOD_DETAIL EPD ON EPD.ENROLLMENT_SETUP_ID = ES.ID
            JOIN ENROLLMENT_WINDOW EW ON EW.ID = EPD.ENROLLMENT_WINDOW_ID
            WHERE C.DELETED = 0 
	...
	--DBMS_OUTPUT.PUT_LINE(I.FROM_DATE || ' <> ' || I.TO_DATE || ' <> ' || I.EFF_DATE || ' <> ' || I.END_DATE);
		...
		SELECT COUNT(*) INTO V_TOTAL_EMPLOYEES FROM EMPLOYEE WHERE EMPLOYER_ORGANIZATION_ID = I.ORG_ID AND DELETED = 0 AND STATUS IN (1); --COUNTING ONLY ACTIVE EMPLOYEES
		SELECT COUNT(*) INTO V_ENROLLED_EMPLOYEES FROM ENROLLMENT E JOIN STATUS_TYPE ST ON E.STATUS_TYPE_ID = ST.ID
			WHERE E.DELETED = 0
